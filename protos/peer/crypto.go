...
func VectorPCommitToken(value []*big.Int, r []*big.Int, PK []ECPoint) (ECPoint, ECPoint) {
	//R := make([]*big.Int, EC.V)
	...
	for i := 0; i < EC.V; i++ {
		...
		modValue := new(big.Int).Mod(value[i], EC.N)
		rmodValue := new(big.Int).Mod(r[i], EC.N)
		// mG, rH
		lhsX, lhsY := EC.C.ScalarMult(EC.BPG[i].X, EC.BPG[i].Y, modValue.Bytes())
		rhsX, rhsY := EC.C.ScalarMult(EC.BPH[i].X, EC.BPH[i].Y, rmodValue.Bytes())
		commitment = commitment.Add(ECPoint{lhsX, lhsY}).Add(ECPoint{rhsX, rhsY})
		trhsX, trhsY := EC.C.ScalarMult(PK[i].X, PK[i].Y, rmodValue.Bytes())
		Token = Token.Add(ECPoint{trhsX, trhsY})
	}

	return commitment, Token
}

func PCommitToken(value *big.Int, r *big.Int, PK ECPoint) (ECPoint, ECPoint) {
	//modValue := new(big.Int).Mod(value, EC.N)
	//rmodValue := new(big.Int).Mod(r, EC.N)
...
type ZKElement struct {
	Commitment, Token ECPoint
	RP                RangeProof
}

func GetVectorR(num int) (r [][]*big.Int) {
	R := make([][]*big.Int, num) //row
	for j := range R {
		R[j] = make([]*big.Int, EC.V) //col
	}
	rsum := make([]*big.Int, EC.V)
	for j := 0; j < EC.V; j++ {
		rsum[j] = big.NewInt(0)
		for i := 0; i < num-1; i++ {
			r, err := rand.Int(rand.Reader, EC.N)
			check(err)
			R[i][j] = r
			rsum[j] = rsum[j].Add(rsum[j], r)
			rsum[j] = new(big.Int).Mod(rsum[j], EC.N)
		}
	}
	for j := 0; j < EC.V; j++ {
		R[num-1][j] = new(big.Int).Sub(EC.N, rsum[j])
	}
	return R
}

func GetR(num int) (r []*big.Int) {
	R := make([]*big.Int, num) //row
	rsum := big.NewInt(0)
	for i := 0; i < num-1; i++ {
		r, err := rand.Int(rand.Reader, EC.N)
		check(err)
		R[i] = r
		rsum = rsum.Add(rsum, r)
		rsum = new(big.Int).Mod(rsum, EC.N)
	}
	R[num-1] = new(big.Int).Sub(EC.N, rsum)
	return R
}

func VectorNIZKCommit(value []*big.Int, PK []ECPoint, index int, num int, r *big.Int) ECPoint {
	...
	for i := 0; i < EC.V; i++ { //currently, EC.V == 1, todo for the high-dimensional cases

		modValue := new(big.Int).Mod(value[i], EC.N)
		rmodeValue := new(big.Int).Mod(r, EC.N)

		// mG, rH
		lhsX, lhsY := EC.C.ScalarMult(EC.BPG[i].X, EC.BPG[i].Y, modValue.Bytes())
		rhsX, rhsY := EC.C.ScalarMult(PK[index].X, PK[index].Y, rmodeValue.Bytes())

		//commitment = commitment.Add(ECPoint{lhsX, lhsY})
		commitment = commitment.Add(ECPoint{lhsX, lhsY}).Add(ECPoint{rhsX, rhsY})

		negR := new(big.Int).Neg(r)
		numB := big.NewInt(int64(num))
		divValue := new(big.Int).Div(negR, numB)
		//fmt.Println("divValue = ", divValue)
		//power := DivMod(negR, numB, EC.N)
		power := new(big.Int).Mod(divValue, EC.N)
		for k := range PK {
			rrhsX, rrhsY := EC.C.ScalarMult(PK[k].X, PK[k].Y, power.Bytes())
			commitment = commitment.Add(ECPoint{rrhsX, rrhsY})
		}
	}
...
//this is only for 1-dimension, not for vector
func PublicKeyPC(SK *big.Int) ECPoint {
	modSK := new(big.Int).Mod(SK, EC.N)
	publickey := EC.H.Mult(modSK)
	return publickey
}

func VectorPublicKeyPC(SK []*big.Int) ECPoint {
	publickey := EC.Zero()
	for i := 0; i < EC.V; i++ {

		modSK := new(big.Int).Mod(SK[i], EC.N)

		// mG, rH
		lhsX, lhsY := EC.C.ScalarMult(EC.BPH[i].X, EC.BPH[i].Y, modSK.Bytes())
		//rhsX, rhsY := EC.C.ScalarMult(EC.BPH[i].X, EC.BPH[i].Y, r.Bytes())

		publickey = publickey.Add(ECPoint{lhsX, lhsY})
	}
	return publickey
}

func VectorPublicKey(SK []*big.Int) ECPoint {
	publickey := EC.Zero()
	for i := 0; i < EC.V; i++ {
		...
		// mG, rH
		lhsX, lhsY := EC.C.ScalarMult(EC.BPG[i].X, EC.BPG[i].Y, modSK.Bytes())
		//rhsX, rhsY := EC.C.ScalarMult(EC.BPH[i].X, EC.BPH[i].Y, r.Bytes())
...
func RPProveNew(v *big.Int, gamma *big.Int, alpha *big.Int, rho *big.Int, sL []*big.Int, sR []*big.Int, tau1 *big.Int, tau2 *big.Int) RangeProof {
	...
	// largest := new(big.Int).Exp(big.NewInt(2), big.NewInt(int64(EC.V)), EC.N)
	// v := new(big.Int).Mod(vorg, largest)
	...
	chal1s256 := sha256.Sum256([]byte(A.X.String() + A.Y.String())) //get the sha256 checksum of (A.X + A.Y)
	cy := new(big.Int).SetBytes(chal1s256[:])                       //challenge y

	rpresult.Cy = cy

	chal2s256 := sha256.Sum256([]byte(S.X.String() + S.Y.String()))
	cz := new(big.Int).SetBytes(chal2s256[:]) //challenge z

	rpresult.Cz = cz
	z2 := new(big.Int).Exp(cz, big.NewInt(2), EC.N)
	// need to generate l(X), r(X), and t(X)=<l(X),r(X)>

	PowerOfCY := PowerVector(EC.V, cy) //y^n
	// fmt.Println(PowerOfCY)
	l0 := VectorAddScalar(aL, new(big.Int).Neg(cz)) //aL-z
	// l1 := sL
	r0 := VectorAdd(
		VectorHadamard(
			PowerOfCY,
		...
		ScalarVectorMul(
			PowerOfTwos,
			z2)) //(y^n)o(aR+z) + (2^n * z2)
	r1 := VectorHadamard(sR, PowerOfCY) //(sR)o(y^n)

	//calculate t0
	t0 := new(big.Int).Mod(new(big.Int).Add(new(big.Int).Mul(v, z2), Delta(PowerOfCY, cz)), EC.N) //v*z^2 + Delta(y^n, z)

	t1 := new(big.Int).Mod(new(big.Int).Add(InnerProduct(sL, r0), InnerProduct(l0, r1)), EC.N) //<sL, r0> + <l0, r1> = <sL, >
	t2 := InnerProduct(sL, r1)                                                                 //<sL,r1>
	...
	cx := new(big.Int).SetBytes(chal3s256[:]) //challenge x
	...
	taux1 := new(big.Int).Mod(new(big.Int).Mul(tau2, new(big.Int).Mul(cx, cx)), EC.N)       //tau2*(x^2)
	taux2 := new(big.Int).Mod(new(big.Int).Mul(tau1, cx), EC.N)                             //tau1*x
	taux3 := new(big.Int).Mod(new(big.Int).Mul(z2, gamma), EC.N)                            //z2*gamma
	taux := new(big.Int).Mod(new(big.Int).Add(taux1, new(big.Int).Add(taux2, taux3)), EC.N) //tau2*(x^2) + tau1*x + z2*gamma
...
func RPProve(v *big.Int) RangeProof {
	...
	*/
	...
	//fmt.Println("sL = ", sL, " r0 = ", r0, " l0 = ", l0, " r1 = ", r1)
...
func NewECPrimeGroupKey(n int) CryptoParams {
	curValue := btcec.S256().Gx //koblitz curves (secp256k1)
	s256 := sha256.New()
